...
| **Namespace-scoped command-channel SAS** | `SasTokenFactory` mints a single namespace-scoped SAS (`https://fqdn/`) for both the request and reply queues, because the collector's `ServiceBusClient(fqdn, AzureSasCredential)` validates the SAS against the namespace URI, not the individual queue. Production should issue separate per-queue tokens and scope them to the entity URI. |
| **SB queue name separator** | Azure Service Bus normalises `/` to `~` in queue names. Command and reply queue names use `~` as separator (`cmd~{tenantId}~{collectorId}`, `reply~{tenantId}~{collectorId}`). Production code should avoid embedding separators by using the entity management API. |
